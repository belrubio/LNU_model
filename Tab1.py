...
class Tab1(tk.Frame):
    def __init__(self, master, phase):
        ...
        self.center = tk.Frame(self, bg='gray97', width=1360, height=600, padx=3, pady=3)
        ...
        self.ctr_mid_mid = tk.Frame(self.center, bg='white', width=640, height=400, padx=3, pady=3)
        self.ctr_mid_right = tk.Frame(self.center, bg='mint cream', width=700, height=400, padx=3, pady=3)
        self.ctr_mid_mid.grid(row=1, column=1, sticky="nsew")
        self.ctr_mid_right.grid(row=1, column=2, sticky="nsew")



        model_label = tk.Label(ctr_mid, text=self.entryFields[0], font=("Helvetica", 16), anchor=tk.W, justify=tk.LEFT, bg='mint cream')
        self.labels=[]
        for i in range(len(self.entryFields)-1):
            self.labels.append (tk.Label(ctr_mid, text=self.entryFields[i+1], bg='mint cream', anchor=tk.W))
        ...
        self.v = [tk.StringVar(ctr_mid, value='0.002'), tk.StringVar(ctr_mid, value='0.01'), 
            ...
            tk.StringVar(ctr_mid, value='0'), tk.StringVar(ctr_mid, value='0'), tk.StringVar(ctr_mid, value='0')]
        self.entries=[]
        for i in range(len(self.entryFields)-1):
            self.entries.append(tk.Entry(ctr_mid, textvariable=self.v[i], background="LightBlue3", width=6))


        self.paramsRehab = self.getParamsRehab(phase)
        ...
        self.labelR3= tk.Label(ctr_right, text='Mean error L hand: -              ', anchor=tk.W, justify=tk.LEFT, bg='LightCyan2')
        self.labelR4= tk.Label(ctr_right, text='Mean error R hand: -', anchor=tk.W, justify=tk.LEFT, bg='LightCyan2')
        self.labelR5= tk.Label(ctr_right, text='Mean cost L hand: -', anchor=tk.W, justify=tk.LEFT, bg='LightCyan2')
        self.labelR6= tk.Label(ctr_right, text='Mean cost R hand: -', anchor=tk.W, justify=tk.LEFT, bg='LightCyan2')
        self.labelR7= tk.Label(ctr_right, text='Mean EReward L hand: -', anchor=tk.W, justify=tk.LEFT, bg='LightCyan2')
        self.labelR8= tk.Label(ctr_right, text='Mean EReward R hand: -', anchor=tk.W, justify=tk.LEFT, bg='LightCyan2')
    ...
    def drawTrialData(self, trialNum, rt, angle_per_trial, listOfAngles, probability_right, probability_left, error_right, error_left, error_extent_left, error_extent_right, expectedRewardLeft, expectedRewardRight, energy_R, energy_L, sensit_R, sensit_L):
        ...
        self.labelR2["text"] = 'P(R Hand selection): ' + str(round(np.mean(probability_right),3))
        self.labelR3["text"] = 'Mean error L hand: ' + str(round(np.mean(error_right)*100,3))
        self.labelR4["text"] = 'Mean error R hand: ' + str(round(np.mean(error_left)*100,3))
        self.labelR5["text"] = 'Mean cost L hand: ' + str(round(np.mean(energy_R),3))
        self.labelR6["text"] = 'Mean cost R hand: ' + str(round(np.mean(energy_L),3))
        self.labelR7["text"] = 'Mean EReward L hand: ' + str(round(np.mean(expectedRewardRight),3))
        self.labelR8["text"] = 'Mean EReward R hand: ' + str(round(np.mean(expectedRewardLeft),3))
        ...
        ax.set_yticklabels([])
        ax.plot(np.radians(listOfAngles), energy_L, 'r', linewidth=1, linestyle='solid')  
        ax.plot(np.radians(listOfAngles), energy_R, 'b', linewidth=1, linestyle='solid')
        ...
        binSet=np.degrees(np.append(np.arange(0,(2*np.pi),(2*np.pi)/12), [(2*np.pi)] ))
        ...
        bin_meansL=[]
        bin_meansR=[]
        bin_meansL, bin_edges, binnumber = stats.binned_statistic(np.radians(listOfAngles), expectedRewardLeft, statistic='mean', bins=12) 
        #bins = np.histogram(expectedRewardLeft, bins=binSet, density=True)

        bin_meansR, bin_edges, binnumber = stats.binned_statistic(np.radians(listOfAngles), expectedRewardRight, statistic='mean', bins=12) 

        #bin_meansL = np.array( [bin_meansL], bin_meansL[0])
        
        #DataHistB, bins = np.histogram(expectedRewardRight, bins=binSet, density=True)
        #ax3.plot(ws, bin_meansL, 'r', linewidth=1, linestyle='solid')  
        #ax3.plot(ws, bin_meansR, 'b', linewidth=1, linestyle='solid')
        ax3.set_yticklabels([])
        # Fill area
        ...
        self.fig4 = plt.figure(figsize=(2.4,3.2), dpi=80, facecolor='w', edgecolor='w')
        ...
        #windrose like a stacked histogram with normed (displayed in percent) results
        self.fig5 = plt.figure(figsize=(2.4,3.2), dpi=80, facecolor='w', edgecolor='w')
        self.fig5.suptitle('\nDirection-tuned cells\n Right Cortex')
    ...
    def runSimulation(self):
        ...
        print('PARAMSREHAB')
        print(self.paramsRehab)

        done = SimulationsMain(self, float(self.entries[0].get()), float(self.entries[1].get()), float(self.entries[2].get()),
        float(self.entries[3].get()), float(self.entries[4].get()), int(self.entries[5].get()), int(self.entries[6].get()),
        int(self.entries[7].get()), self.simulateStroke, self.simulateRehab, self.simulateFU, self.paramsRehab[0], self.paramsRehab[1], self.paramsRehab[2])
    ...
    def play_trial(self, armAngleG= 0, foreArmAngleG = 0, targetXG = 0 ,targetYG =0  , selectedHand=-1, acR=0, acL=0, pacR=0, pacL=0, ac=0, expR_R=0, expR_L=0, startTime=0, currentT=0, trialNow=0, flag=False):
        if (trialNow==int(self.entries[6].get())+5) or (trialNow==int(self.entries[7].get())+5):
            pl.close_gui()
        else:
    ...
    def getParamsRehab(self, phase):
        return {
            'Training': [0,0,0],
            'Stroke': [0,0,0],
            'Rehab': [0,0,0], #[float(self.entries[8].get()),float(self.entries[9].get()),float(self.entries[10].get())],
            'Followup': [0,0,0]
        }.get(phase, [0,0,0])
        
    def getEntryFields(self, phase):
        return {
            'Training': ['Free Parameters', 'UD Learning Rate:', 'EB Learning Rate:', 'RB Learning Rate:', 'Exploration Level:',
            'NP Arm Bias:', 'Number of Trials:', 'Show 5 trials at:', '& at'],
            'Stroke': ['Free Parameters', 'UD Learning Rate:', 'EB Learning Rate:', 'RB Learning Rate:', 'Exploration Level:',
            'NP Arm Bias:', 'Number of Trials:', 'Show 5 trials at:', '& at'],
            'Rehab': ['Free Parameters', 'UD Learning Rate:', 'EB Learning Rate:', 'RB Learning Rate:', 'Exploration Level:',
            'NP Arm Bias:', 'Number of Trials:', 'Show 5 trials at:', '& at', 'Force:', 'Gain:', 'Steer:'],
            'Followup': ['Free Parameters', 'UD Learning Rate:', 'EB Learning Rate:', 'RB Learning Rate:', 'Exploration Level:',
            'NP Arm Bias:', 'Number of Trials:', 'Show 5 trials at:', '& at']
        }.get(phase, ['Free Parameters', 'UD Learning Rate:', 'EB Learning Rate:', 'RB Learning Rate:', 'Exploration Level:',
            'NP Arm Bias:', 'Number of Trials:', 'Show 5 trials at:', '& at'])
